new Vue({
    ...
    mounted() {
        // 尝试从本地存储加载题目
        const savedQuestions = localStorage.getItem('training-questions');
        if (savedQuestions) {
            this.questions = JSON.parse(savedQuestions);
        } else {
            // 如果没有保存的题目，使用示例数据
            this.questions = window.trainingQuestions || [];
        }
    ...
    methods: {
        ...
        saveQuestions() {
            localStorage.setItem('training-questions', JSON.stringify(this.questions));
        },
